...
namespace EGamePlay.Combat
{
    ...
    public class DamageAction : ActionExecution
    {
        ...
        private void PreProcess()
        {
            ...
            {
                IsCritical = (RandomHelper.RandomRate() / 100f) < Creator.UnitPropertyEntity.CriticalChance.Value;
                DamageValue = Mathf.CeilToInt(Mathf.Max(1, Creator.UnitPropertyEntity.AttackPower.Value - Target.UnitPropertyEntity.Shield.Value));
                if (IsCritical)
                {
            ...
            if (DamageSource == DamageSource.Skill)
            {
                ...
                DamageValue = Mathf.CeilToInt(Mathf.Max(1, AbilityEffect.GetComponent<EffectDamageComponent>().GetDamageValue() - Target.UnitPropertyEntity.Shield.Value));
                if (IsCritical)
                {
                    DamageValue = Mathf.CeilToInt(DamageValue * 1.5f);
                }
