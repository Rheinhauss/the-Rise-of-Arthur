    ...
    public partial class SkillExecution
    {
        ...
        public override void BeginExecute()
        {
            ...
            if (timelineAsset == null)
                return;
            var skillExecutionObj = GameObject.Instantiate(SkillExecutionAsset, InputPoint, Quaternion.Euler(InputDirection));
        ...
        private void ForwardFlyProccess(ExecutionEventEmitter colliderSpawnEmitter)
        {
            ...
            abilityItem.Position = InputPoint;
            CreateAbilityItemObj(abilityItem);
            var x = Mathf.Sin(Mathf.Deg2Rad * InputDirection.y);
            var z = Mathf.Cos(Mathf.Deg2Rad * InputDirection.y);
        ...
        private void FixedDirectionProccess(ExecutionEventEmitter colliderSpawnEmitter)
        {
            ...
            abilityItem.Name = colliderSpawnEmitter.ColliderName;
            abilityItem.Position = InputPoint;
            abilityItem.Direction = InputDirection;
            CreateAbilityItemObj(abilityItem);
            abilityItem.AddComponent<LifeTimeComponent>(colliderSpawnEmitter.ExistTime);
        }
        ...
        {
            this.SkillAbility.ApplyEffectsToSelf();
            var abilityItemObj = GameObject.Instantiate(Resources.Load<GameObject>($"Skills/AbilityItems/{abilityItem.Name}"), abilityItem.Position, Quaternion.Euler(abilityItem.Direction));
