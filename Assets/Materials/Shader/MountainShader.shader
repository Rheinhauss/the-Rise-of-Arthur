    ...
    SubShader 
	{
		...
		Pass 
		{
			...
			{ 
				...
				//sum = color * sum.r;
				//if (sum.r < color.r) sum.r = color.r;
				//if (sum.g < color.g) sum.g = color.g;
				//if (sum.b < color.b) sum.b = color.b;

				if (sum.r > 0.7)
				{
					sum = sum.r * pow(color, 1.0);
				}
				else if (sum.r > 0.3) {
					sum = sum.r * pow(color, 1.5);
				}
				else {
					sum = sum.r * pow(color, 2.0);
				}
				//sum = color * sum;
